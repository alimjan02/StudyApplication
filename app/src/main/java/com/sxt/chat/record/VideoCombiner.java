...
public class VideoCombiner {
    ...
    public interface VideoCombineListener {
         ...
         * @param mDestPath 合并成功后的音频文件
         */
        void onCombineFinished(boolean success, String mDestPath);
    }
    ...
    public void combineVideo() {
        ...
        if (!hasVideoFormat && !hasAudioFormat) {
            if (mCombineListener != null) {
                mCombineListener.onCombineFinished(false, null);
            }
            return;
        }
    ...
    private int selectTrack(MediaExtractor extractor, String mimePrefix) {
        ...
        for (int i = 0; i < numTracks; i++) {
            if (extractor.getTrackFormat(i).getString(MediaFormat.KEY_MIME).startsWith(mimePrefix)) {
                audioIndex = i;
                break;
            }
        }
